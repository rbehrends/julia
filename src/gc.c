...
// of objects.
static int support_conservative_marking;
...
JL_DLLEXPORT int jl_gc_enable_conservative_gc_support()
{
    ...
    if (jl_is_initialized()) {
        ...
        if (!result) {
            // Reset age bits. 
            // We don't have to worry about race conditions for this
            // part, as allocation itself is unproblematic and a
            // collection will wait for safepoints.
            jl_gc_collect(1);
        }
        return result;
    } else {
