...
JL_DLLEXPORT jl_value_t *jl_gc_big_alloc(jl_ptls_t ptls, size_t sz)
{
    ...
    for (jl_gc_hooks_t *h = jl_gc_hooks; h && h->register_bigval; h = h->next)
        h->register_bigval(v, sz);
    if (v == NULL)
        jl_throw(jl_memory_exception);
#ifdef JULIA_ENABLE_THREADING
...
#endif
            for (jl_gc_hooks_t *h = jl_gc_hooks; h && h->deregister_bigval; h = h->next)
                h->deregister_bigval(v);
            jl_free_aligned(v);
        }
...
mark: {
        ...
        if (vt == jl_simplevector_type) {
            ...
            for (jl_gc_hooks_t *h = jl_gc_hooks; h && h->task_scanner_hook; h = h->next)
                h->task_scanner_hook(ta, ta == ptls2->root_task);
            if (stkbuf) {
#ifdef COPY_STACKS
...
static int _jl_gc_collect(jl_ptls_t ptls, int full)
{
    ...
    for (jl_gc_hooks_t *h = jl_gc_hooks; h && h->root_scanner_hook; h = h->next)
        h->root_scanner_hook(full);
...
{
    for (jl_gc_hooks_t *h = jl_gc_hooks; h && h->pre_gc_hook; h = h->next)
        h->pre_gc_hook(full);
    jl_ptls_t ptls = jl_get_ptls_states();
    ...
    for (jl_gc_hooks_t *h = jl_gc_hooks; h && h->post_gc_hook; h = h->next)
        (*h->post_gc_hook)(full);
}
...
#endif
    gc_current_sp = (gc_mark_sp_t **)jl_malloc_aligned(jl_n_threads * sizeof(gc_mark_sp_t *), 64);
